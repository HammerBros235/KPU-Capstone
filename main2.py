    ...
    def startVideo(self):
        ...
        while run_video:  # 이상태님 코드 결합
            ...
            for (x, y, w, h) in faces:
                ...
                if (point_index>0):
                    p1= Point(points[point_index-1][0],points[point_index-1][1],points[point_index-1][2],points[point_index-1][3])
                    p2 = Point(points[point_index][0],points[point_index][1],points[point_index][2],points[point_index][3])
                    pD = pDiff(p1,p2)
                point_index += 1
            ...
            # time값이 0이면 무한 대기, waitKey는 키가 입력 받아 질때까지 기다리는 시간을 의미한다.
...
class ImageViewer(QWidget):
    ...
    def paintEvent(self, event): # 실제 사각형을 그리는 함수, 이벤트를 받을때마다 동작하는 것 같습니다. 
        #다만, 얼굴, 눈 구별하지 않고 모든 이벤트에 대해서 받는 것 같은데 어떻게 구별할지는 잘 모르겠습니다.
